...
def train_models(pool_data, xvec_out_dir, output_x_vec_select, combine_genders=False):
    ...
    if combine_genders:
        ...
        for gender in ('m', 'f'):
            ...
            clustering[gender] = KMeans(n_clusters=int(clusters_per_conv/2), random_state=random_seed).fit(gender_xvecs)

            closest, _ = pairwise_distances_argmin_min(clustering[gender].cluster_centers_, gender_xvecs)
            print(gender)

            for spk_close_centroid in closest:
                xvec_1 = gender_xvecs[spk_close_centroid]

                with ReadHelper('scp:'+pool_xvec_file) as reader:
                    for key, xvec in reader:
                        if (xvec_1 == xvec).all():
                            print("Spk:" , key, "closest spk to centroid_id:", np.where(spk_close_centroid == closest))
                            kaldiio.save_ark(f"{output_x_vec_select}/spk_list.ark", {f"{key}": xvec}, append=True,  scp=f"{output_x_vec_select}/spk_list.scp")
                            break


            i = 0
            for (xvec, label) in zip(gender_xvecs, clustering[gender].labels_):
                kaldiio.save_ark(f"{output_x_vec_select}/{gender}{label}.ark", {f"{i}": xvec}, append=True,  scp=f"{output_x_vec_select}/{gender}{label}.scp")
                i += 1
