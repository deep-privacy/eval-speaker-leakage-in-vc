...
if [ $stage -le 2 ]; then
  ...
  new_data_dir=data/${src_data}_anon
  ...
  cp ${anon_xvec_out_dir}/xvectors_${src_data}/pseudo_xvecs/spk2gender ${new_data_dir}/
  ...
  if [ -f data/$src_data/enrolls ]; then
    ...
    [ ! -f data/$src_data/trials ] && echo "File data/$src_data/trials does not exist" && exit 1
